...
open struct
  ...
  let unify0 =
    test_case "unify" `Quick (fun () ->
      ...
      and computed = Kind.unify Nel.(singleton Kind.bool) in
  ...
  let unify1 =
    test_case "unify" `Quick (fun () ->
      let expected = Kind.bool
      and computed = Kind.(unify Nel.(bool :: [ bool ])) in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
  ...
  let unify2 =
    test_case "unify" `Quick (fun () ->
      ...
      and computed = Kind.(unify Nel.(bool :: [ bool; string; any ])) in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
  ...
  let unify3 =
    test_case "unify2" `Quick (fun () ->
      ...
      and computed =
        Kind.(unify Nel.(bool :: [ bool; string; int; string; null ]))
      in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
  ...
  let unify4 =
    test_case "unify" `Quick (fun () ->
      let expected =
        Kind.(unify Nel.(bool :: string :: [ int; or_ int string ]))
      and computed =
        Kind.(unify Nel.(bool :: [ bool; string; int; or_ int string ]))
      in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
  ...
  let sum0 =
    test_case "sum" `Quick (fun () ->
      ...
      and computed = Kind.(sum Nel.[ "foo", bool ]) in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
  ...
  let sum1 =
    test_case "sum" `Quick (fun () ->
      let expected =
        Kind.(unify Nel.(branch "foo" bool :: [ branch "bar" int ]))
      and computed = Kind.(sum Nel.[ "foo", bool; "bar", int ]) in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
          ...
          sum
            Nel.
              ...
              ; ( "foobar"
                , record
                    [ "name", string
                    ; "policies", list (triple int float string)
                    ] )
              ])
      in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ...
  let tuple0 =
    test_case "tuple" `Quick (fun () ->
      ...
      and computed =
        Kind.tuple Nel.(from_list_exn Kind.[ int; float; bool; string ])
      in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
  ...
  let tuple1 =
    test_case "tuple" `Quick (fun () ->
      ...
      and computed = Kind.tuple Nel.(from_list_exn Kind.[ int ]) in
      check Test_lib.Testable.kind "should be equal" expected computed)
  ;;
end
